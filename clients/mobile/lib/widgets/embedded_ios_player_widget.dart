...
/// Wraps a [UiKitView] that hosts a native [AVPlayerLayer] surface, allowing
/// video to render **inline** inside the Flutter widget tree on iOS.
///
/// One per-view [MethodChannel] (named
/// `com.x87player/ios_player_view/<viewId>`) is established after the
...
/// callback.
///
/// On non-iOS platforms the widget falls back to an expanded [SizedBox] —
/// the caller ([VlcPlayerWidget]) is expected to handle the fallback UI.
class EmbeddedIosPlayerWidget extends StatefulWidget {
  const EmbeddedIosPlayerWidget({
    super.key,
    required this.url,
    required this.title,
  ...
  @override
  State<EmbeddedIosPlayerWidget> createState() =>
      _EmbeddedIosPlayerWidgetState();
}

class _EmbeddedIosPlayerWidgetState extends State<EmbeddedIosPlayerWidget> {
  static const String _viewType = 'com.x87player/ios_player_view';
  ...
  void _onPlatformViewCreated(int viewId) {
    final ch = MethodChannel('com.x87player/ios_player_view/$viewId');
            ...
            child: Column(
              ...
              children: [
                const Icon(Icons.error_outline,
                    color: Color(0xFFE74C3C), size: 36),
